...
function Warning()
    if not HasPlugin("vnavmesh") then
        yield("/echo [Artisan] Please Install vnavmesh")
    end

    if not HasPlugin("PandorasBox") then
        yield("/echo [Artisan] Please Install Pandora's Box")
    end

    if not HasPlugin("Artisan") then
        yield("/echo [Artisan] Please Install Artisan")
    end

    if not HasPlugin("YesAlready") then
        yield("/echo [Artisan] Please Install YesAlready")
    end

    if not HasPlugin("Lifestream") then
        yield("/echo [Artisan] Please Install Lifestream")
    end

    if not HasPlugin("AutoRetainer") then
        yield("/echo [Artisan] Please Install AutoRetainer")
    end
end
