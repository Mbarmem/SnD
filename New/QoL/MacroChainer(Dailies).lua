...
function OnChatMessage()
    ...
    if currentEchoTrigger and message:find(currentEchoTrigger) then
        LogInfo(string.format("%s Detected echo for: %s | Type: %s | Sender: %s | Message: %s", EchoPrefix, currentEchoTrigger, tostring(messageType), tostring(sender), tostring(message) ))
        macroDone = true
    end
end
...
for _, macro in ipairs(MacrosToRun) do
    ...
    LogInfo(string.format("%s Starting macro: %s", EchoPrefix, macro.macroName))
    ...
    LogInfo(string.format("%s Completed macro: %s", EchoPrefix, macro.macroName))
    yield("/wait 1")
end
...
LogInfo(string.format("%s All macros completed. Stopping any remaining..!!", EchoPrefix))
StopRunningMacros()
