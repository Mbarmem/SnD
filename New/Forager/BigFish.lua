...
--        - x, y            <- FISHING_SPOTS[location].map_coords (already in
--                             normal in-game map-coordinate units, no conversion
--                             needed - fed straight into SetMapFlag)
...
--   CharacterState.travelToSpot   - SetMapFlag(x, y, true) -> Mount() ->
--                                  WaitForNavMesh() -> Execute("/vnav flyflag") ->
--                                  WaitForPathRunning() -> Dismount(). Uses the
--                                  map-coordinate flag + vnavmesh's own "flyflag"
--                                  command specifically so no world-coordinate
--                                  (X/Y/Z) conversion is ever needed - same
--                                  pattern as QoL/Alexandrite.lua.
...
--   - Never tested in-game end-to-end (movement/navmesh behavior at these exact
--     spots, AutoHook preset stop-on-catch behavior, and the weather/time math
--     itself all still need real-world verification).
...
function CharacterState.travelToSpot()
    ...
    LogInfo(string.format("%s Flying to %s (%.1f, %.1f)", LogPrefix, SelectedFish.spotName, SelectedFish.x, SelectedFish.y))
    SetMapFlag(SelectedFish.x, SelectedFish.y, true)
    Mount()
    WaitForNavMesh()
